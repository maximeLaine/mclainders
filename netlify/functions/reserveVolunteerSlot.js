...
const MSG_INVALID_TABLE = 'Table invalide.';
...
const MSG_SLOT_ALREADY_RESERVED = 'Cette place a déjà été réservée. Veuillez en choisir une autre.';
const MSG_AVAILABILITY_ERROR = 'Erreur lors de la vérification de la disponibilité.';
const MSG_RESERVATION_ERROR = 'Erreur lors de la réservation.';
const MSG_SUCCESS = 'Votre place a été réservée avec succès!';
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { name, email, spotIndex, tableName } = data;

    if (!name || !email || spotIndex === undefined || !tableName) {
      return {
    ...
    if (!VALID_TABLES.includes(tableName)) {
      return {
        statusCode: STATUS_BAD_REQUEST,
        headers,
        body: JSON.stringify({ success: false, message: MSG_INVALID_TABLE }),
      };
    }
    ...
    if (updateError) {
      console.error('Error reserving slot:', updateError);
      return {
        statusCode: STATUS_SERVER_ERROR,
        headers,
